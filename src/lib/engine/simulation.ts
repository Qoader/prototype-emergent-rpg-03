...
const gridOf = (point: WorldPoint): GridPoint => ({
...
function emptyNavigation(): NavigationState {
  return { waypoints: [], moving: false };
}
  ...
  const entities: Entity[] = [
    {
      ...
      kind: 'hero',
      name: 'The Wayfarer',
      position: center(capital.position),
...
function routeScore(entity: Entity, anchor: GridPoint, route: GridPoint[]): number {
  return (
    edgeDistance(entity.position, anchor) +
    route.reduce((score, point, index) => {
...
): { anchor: GridPoint; route: GridPoint[] } | undefined {
  return anchorsFor(entity)
    .map((anchor, index) => ({ anchor, index, route: findPath(state.map, anchor, destination) }))
    .filter(({ anchor, route }) => same(anchor, destination) || route.length > 0)
    .sort(
      (left, right) =>
        routeScore(entity, left.anchor, left.route) -
          routeScore(entity, right.anchor, right.route) || left.index - right.index,
    )[0];
}
...
function setRoute(entity: Entity, anchor: GridPoint, route: GridPoint[]): void {
  const next = route[0];
  if (!next) {
    entity.navigation = { waypoints: [], moving: false };
    return;
  }
  entity.navigation = {
    edge: { a: anchor, b: next, target: next },
...
function beginRoute(state: SimulationState, entity: Entity, destination: GridPoint): boolean {
  ...
  if (same(choice.anchor, destination)) {
    entity.position = center(destination);
    entity.navigation = emptyNavigation();
    return true;
  }
...
function stopEntity(entity: Entity): void {
  entity.navigation.waypoints = [];
...
function footprintTargets(state: SimulationState, resident: Entity): GridPoint[] {
  ...
  const settlement = state.map.settlements.find(
    (candidate) => candidate.id === homeTile?.settlementId || same(candidate.position, home),
  );
  if (!settlement) return [];
  let bySettlement = footprintCache.get(state.map);
  if (!bySettlement) {
    bySettlement = new Map();
    footprintCache.set(state.map, bySettlement);
  }
  const cached = bySettlement.get(settlement.id);
  if (cached) return cached;
  const targets = state.map.tiles
    .filter(
      (tile) =>
        tile.walkable && (tile.settlementId === settlement.id || same(tile, settlement.position)),
    )
    .map(({ x, y }) => ({ x, y }))
...
function chooseResidentTarget(state: SimulationState, resident: Entity): void {
  ...
  const localTargets = footprintTargets(state, resident).filter(
    (target) => !same(target, currentTile) && distance(target, currentTile) <= 4,
  );
  const targets = localTargets.length
    ? localTargets
    : footprintTargets(state, resident).filter((target) => !same(target, currentTile));
  if (!targets.length) {
    wander.idleRemainingMs = 1000;
    return;
  }
  const random = createRandom(`${state.seed}:${resident.id}:${wander.decisionCount}`);
  const target = targets
    .map((candidate) => ({ candidate, value: random() }))
    .sort((a, b) => a.value - b.value)[0].candidate;
...
function moveOne(entity: Entity, dtSeconds: number): void {
  ...
  while (remaining > EPSILON && entity.navigation.moving && iterations++ < 32) {
    ...
    const distanceToTarget =
      Math.abs(target.x - entity.position.x) + Math.abs(target.y - entity.position.y);
    if (distanceToTarget <= remaining + EPSILON) {
      ...
      if (next) entity.navigation.edge = { a: edge.target, b: next, target: next };
        ...
        entity.navigation.edge = undefined;
        entity.navigation.moving = false;
      }
      continue;
    }
...
export function requestMove(state: SimulationState, destination: GridPoint): boolean {
  ...
  if (same(gridOf(hero.position), destination)) {
    stopEntity(hero);
    return true;
  }
  const target =
    tileAt(state.map, destination)?.walkable && chooseRoute(state, hero, destination)
      ? destination
      : resolveFallback(state, hero, destination);
  if (!target) {
    stopEntity(hero);
    return false;
  }
...
export function advanceSimulation(state: SimulationState, dtSeconds: number): void {
  ...
  let residentDecisionAvailable =
    Math.floor(state.simulationTimeMs / 100) !== Math.floor(previousTimeMs / 100);
